...
import { getSyncHubData, gitCommit, gitPull, gitPush } from '../services/apiService';
import type { SyncHubData, GitData, CloudFile, SyncActivity } from '../types';
import { CommitIcon, GitBranchIcon, GitPullIcon, GitPushIcon, RefreshIcon, FileIcon, ModifiedIcon, AddedIcon, DeletedIcon, RenamedIcon, SyncIcon, DropboxIcon, GistIcon, CheckIcon, PullRequestIcon, GitHubIcon, AddIcon, ChevronDownIcon, ChevronRightIcon } from './Icons';
...
// --- Git Components ---
const GitFileItem: React.FC<{ path: string; status: string; onToggle: () => void; isStaged: boolean; }> = ({ path, status, onToggle, isStaged }) => {
    ...
    return (
        <div className="flex items-center space-x-3 pl-3 pr-1 py-1.5 rounded-md hover:bg-dark-700/50 group">
            ...
            <button onClick={onToggle} className="text-gray-500 hover:text-primary-400 p-1 opacity-0 group-hover:opacity-100">
                    ...
                    <div className="space-y-1 bg-dark-900/50 rounded-md p-1">
                        {filesToDisplay.staged.length > 0 ? filesToDisplay.staged.map((path, i) => (
                           <div key={path} className="animate-stagger-in" style={{animationDelay: `${i * 20}ms`}}>
                             <GitFileItem path={path} status={fileStatusMap[path]} isStaged={true} onToggle={() => toggleStaging(path)} />
                           </div>
                 ...
                 <div>
                    ...
                    <div className="space-y-1 bg-dark-900/50 rounded-md p-1">
                       {filesToDisplay.unstaged.length > 0 ? filesToDisplay.unstaged.map((path, i) => (
                           <div key={path} className="animate-stagger-in" style={{animationDelay: `${i * 20}ms`}}>
                             <GitFileItem path={path} status={fileStatusMap[path]} isStaged={false} onToggle={() => toggleStaging(path)} />
                           </div>
